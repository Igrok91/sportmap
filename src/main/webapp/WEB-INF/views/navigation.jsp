...
<header>
    <div class="btn-group btn-group-justified " role="group" style="height: 60px;box-shadow: 0 0 1px;" id="web">
        <a id="events" class="cursorPointer btn" style="background: whitesmoke"><span
                class="glyphicon glyphicon-calendar" aria-hidden=""></span> События</a>
        <a id="searchPlayground" class="cursorPointer btn" style="background: whitesmoke"><span
                class="glyphicon glyphicon-search" aria-hidden=""></span>
            Площадки</a>
        <a id="create" class="cursorPointer btn " data-toggle="modal"
           data-target="#exampleModal" style="font-size: medium;background: whitesmoke"><span
                class="glyphicon glyphicon-plus-sign"
                aria-hidden="Создать"></span></a>
        <a id="groups" class="cursorPointer btn " style="background: whitesmoke"><span
                class="glyphicon glyphicon-th-list" aria-hidden=""></span> Группы</a>
        <a id="profile" class="cursorPointer btn " style="background: whitesmoke"><span class="glyphicon glyphicon-user"
                                                                                        aria-hidden=""></span>
            Профиль</a>
...
<script>
    ...
    function handleSearchPlayground() {
        document.getElementById("event").className = "hide";
        document.getElementById("prof").className = "hide";
        document.getElementById("group").className = "hide";
        document.getElementById("search").className = "";
        ...
        $('#searchPlayground').addClass('btn-default');
        $('#events').removeClass('active');
        $('#events').removeClass('btn-default');
        $('#groups').removeClass('active');
        $('#groups').removeClass('btn-default');
        $('#profile').removeClass('active');
        $('#profile').removeClass('btn-default');
        $('#create').removeClass('active');
        $('#create').removeClass('btn-default');
    }
    ...
    function handleEvents() {
        ...
        $('#searchPlayground').removeClass('btn-default');
        $('#events').addClass('active');
        $('#events').addClass('btn-default');
        $('#groups').removeClass('active');
        $('#groups').removeClass('btn-default');
        $('#profile').removeClass('active');
        $('#profile').removeClass('btn-default');
